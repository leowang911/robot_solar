...
class BaseSerialNode:
    def __init__(self):
        ...
        #add IO IMU laser encoder
        self.linear_acceleration = [0, 0, 0]  # 假设有三个轴的加速度
        self.angular_velocity = [0, 0, 0]  # 假设有三个轴的角速度
        self.roll = 0
        self.pitch = 0
        self.yaw = 0
        self.yaw_rad = 0  # 用于存储yaw的弧度值
        # math.radians(0) # 转换为弧度
        self.laser_1 = 0   #左右待确定，注释：
        self.laser_2 = 0 
        self.encoder_left = 0
        self.encoder_right = 0 
        self.encoder_brush =0


        self.last_tx_data = None
        self.last_tx_data_prev = {
            'distance': 0,
            'target_yaw': 0, #待确定，谁来传
            'roller_speed': 0,
            'sensor_state': 0,
            'complete_state': 1,  # 初始设置为1
            'rc_state': 0,
            'robot_state': 1,
            'battery': 0,
            'error': 0,
            ...
            'pitch':0,
            'yaw':0,
            'yaw_rad': 0,  # 用于存储当前yaw的弧度值  对应原来current_yaw
            'laser_1': 0,
            'laser_2': 0,
            'encoder_left': 0,
            'encoder_right': 0,
    ...
    def control_data_callback(self, msg):
        ...
        self.last_tx_data = {
            ...
            'yaw': self.yaw_to_target_yaw_angle(self.yaw_rad, 0),
            'robot_state': msg.robot_state
        }
        # rospy.logwarning(f'yaw: {self.yaw}')

    def parse_rx_frame(self, data):
        """解析接收数据帧"""
        try:
            if data[0] != 0xAA or len(data) != self.rx_frame_length or data[35] != sum(data[:35]) & 0xFF:
            ...
            # add io IMU laser encoder  AX[15:0]/32768*16g
            
            # self.roll = struct.unpack('>h', data[13:15])[0] 
            self.roll = struct.unpack('>h', data[13:15])[0] / 32768 * 180  
                            # / 32768 * 16 * 9.8,  # 假设加速度数据在第14-15字节
            self.pitch = struct.unpack('>h', data[15:17])[0] 
            self.yaw = struct.unpack('>h', data[17:19])[0] 
            self.yaw_rad = math.radians(self.yaw / 32768 * 180) # 转换为弧度
            
            self.laser_1 = struct.unpack('>h', data[19:21])[0]  
            self.laser_2 = struct.unpack('>h', data[21:23])[0]  
            self.encoder_left = struct.unpack('>i', data[23:27])[0]  
            self.encoder_right = struct.unpack('>i', data[27:31])[0]
            self.encoder_brush = struct.unpack('>i', data[31:34])[0]  
            ...
            return {
                ...
                'sensor_state': bin(self.sensor_state),
                ...
                'encoder_brush': self.encoder_brush
                }
    ...
    def create_tx_frame(self, data):
        ...
        tx_yaw = self.yaw_to_target_yaw_angle(self.yaw_rad,0)
    ...
    def publish_wheel_status(self, data):
        ...
        msg.sensor_state = bin(data['sensor_state'])
        msg.complete_state = data['complete_state']
        msg.rc_state = data['rc_state']


        # add IMU laser encoder
        msg.linear_acceleration = data['linear_acceleration']
        msg.angular_velocity = data['angular_velocity']
        msg.roll = data['roll']
        msg.pitch = data['pitch']
        msg.yaw = data['yaw']  # 使用当前航向角
        msg.laser_left = data['laser_left']
        msg.laser_right = data['laser_right']
        msg.encoder_left = data['encoder_left'] 
        msg.encoder_right = data['encoder_right']
