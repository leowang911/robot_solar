...
class ArucoDockingController:
    def __init__(self):
        ...
        self.stop_refine_pose_dlt_y=0.05
...
#------------------------------------CALCULATION---------------------------------------------------------------------------------------------------
    ...
    def estimate_center(self, side):
        ...
        self.pos_target = R@[-sign*2.5,0,0.3] + pos
    ...
    def get_five_avg(self,id):
        ...
        for i in range(11):
            ...
            if id == 'side_target':
                loc=copy.deepcopy(self.side_target)
        ...
        if self.stop_flag == False: #是否进入停止状态
            ...
            if self.rc_control == 1:
                ...
                if self.docking_flag ==False : #todo 
                    ...
                    if self.distance2drone > 1 and self.current_target is None: #gps距离大于2米,通过gps数据大致导航
                        ...
                        if self.markers['left'] or self.markers['right'] and not (self.markers['center'] or self.markers['center_left'] or self.markers['center_right']):
                            ...
                            rospy.loginfo(f"side_target_yaw: {control.target_yaw},{self.current_yaw}")
                            control.robot_state = 1
                            control.header.stamp = rospy.Time.now()
                            self.control_pub.publish(control)
                            time.sleep(0.05)
                        ...
                        if self.current_target is not None and not (self.markers['left'] or self.markers['right']):
                            ...
                            if self.refine_align==True:
                                        ...
                                        self.control_pub.publish(control)
                                        time.sleep(0.5)
                                        current_pose_state=self.get_five_avg('current_target')#取5次平均值进行计算
                ...
                if self.out_dock_flag == False:
                    ...
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    control = self.compose_control(2000,0,self.current_yaw,math.pi/2*self.sign,2)
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    while self.complete_state !=2:
                        continue
                    ...
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    control = self.compose_control(1000,0,self.current_yaw,-math.pi/2*self.sign,2)
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    while self.complete_state !=2:
                        continue
                    ...
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    control = self.compose_control(0,0,self.current_yaw,-math.pi/2*self.sign,2)
                    self.control_pub.publish(control)
                    time.sleep(0.1)
                    while self.complete_state !=2:
                        continue
