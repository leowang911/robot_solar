...
class ArucoDockingController:
    def __init__(self):
        ...
        self.stop_distance = rospy.get_param('~stop_distance', 1.1)  # 中间标记前停止距离
...
#------------------------------------CALCULATION---------------------------------------------------------------------------------------------------
    ...
    def calculate_center_side_target(self, side):
        ...
        # 计算中间位置 * sign
        offset = self.marker_side_spacing/2 *sign-0.035,0
        self.pos_target = R@np.array([-offset, self.stop_distance]) + pos
